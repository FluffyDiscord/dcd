...
| TC-031 | dotenv parser (Symfony 8.1 port) | the **vendored** `DotenvTest.php` providers, machine-extracted to `tests/fixtures/symfony/dotenv_cases.json` (93 value + 19 error cases) and run from there, plus the hand-written load/loadEnv chain cases | every upstream case passes verbatim: identical values, identical error reason + caret line, window masked 1:1 (`upstream_value_provider_passes_verbatim`, `upstream_error_provider_passes_with_only_value_bytes_masked`) | the 7 completed-`$(cmd)` cases → `ConfigError` instead of execution, listed by exact input so a new upstream one fails loudly; `$_SERVER`/`putenv`/`.env.local.php` N/A (documented); `PASS=pa'ss` literal vs `FOO=va'lue'` concatenated; NUL rejection; BOM; CRLF |
...
| IT-003 | rollback | deploy v1, deploy v2 with `--image`, `dcd rollback it -y`; `migrate:after` leaves a marker inside the release | a new container on **v1's image**, upstream re-pointed to it, the marker absent (no migration ran) with the v2 container's marker as the control, `status` shows `RolledBack` / `Active` | fixture drop-guard |
| IT-004 | resume | deploy, then a deploy that fails in `migrate:after` (exit 4); fix the config; `dcd deploy --resume` | resume finishes **that same container** (identity asserted — a resume that started a new black passes every state-only check), no `cutover_pending` left, state `active`, stage deployable again | as above |
| IT-005 | concurrent lock | (a) the flock held by hand, (b) a real spawned deploy holding it | both refuse the second deploy with exit 3 naming the holder | as above |
| IT-005b | **a killed holder releases the local flock** (INV-4) | spawn a deploy, wait for `.lock.meta`, `SIGKILL` it | the flock is free — asked of the kernel (`try_lock` here) and of dcd (the next deploy runs, no `unlock`) — because the fd died with the process, not because anything swept up | as above |
...
| IT-011 | **the remote lock releases itself** (INV-4) | ssh fixture; start a deploy, wait for `.meta`, `SIGKILL` dcd | the lock (asked of `flock -n` on the target, not of dcd) is free within seconds with **no** `unlock`, and the next deploy simply runs. Variant, its own test: `SIGSTOP` the holder — the ssh child and its channel stay up, only the heartbeat stops — and the lock goes free after the 30 s lease, **not** before (asserted, or the test would pass on a channel that merely closed) | fixture drop-guard; `SIGCONT` + kill |
| IT-011b | **the lock is exclusive while heartbeating** | ssh fixture; a spawned deploy holds the stage, a second one runs against it | the second exits 3 with `another deploy holds prod`, quoting the `.meta` sidecar verbatim — whose `pid` is the first deploy's real pid; the holder then finishes successfully | as above |
| IT-012a | **transport loss pre-cutover** | ssh fixture (sshd is not pid 1, so the target outlives it); a `before_pull` hook runs `pkill sshd` | exit **6** naming the lost connection; the red is still serving and still `current`, with nothing `cutover_pending` | as above |
| IT-012b | **transport loss post-cutover** | ssh fixture; a `before_migrate_after` hook runs `pkill sshd` | exit **4** (phase wins over transport); the black is live, and the target's state file — read over `docker exec`, since ssh is gone — records it `cutover_pending` for `--resume` | as above |
...
| IT-014 | **release container survives compose reconciliation** (INV-13/14) | a fixture with a `worker` service and a static worker set; deploy, then an operator's `docker compose up -d --remove-orphans`, then a second deploy (whose `drain:red` drains workers while the new release is live) | the release container survives both: same id and same `StartedAt` across the compose sweep, still running after the drain. The control is the worker's id **changing**, so "the release survived" is never the answer to a drain that never ran | fixture drop-guard |
...
| INV-3 | TC-015 (record at cutover), TC-020, IT-008 (a stuck stage is recoverable from the record), IT-004 (`--resume` finishes the recorded release), IT-012b (the record survives the link dropping) |
| INV-4 | TC-023, IT-005 + IT-005b (local flock: exclusive, and reclaimed when the holder is killed); IT-011 (remote lease: released on SIGKILL, and on lease expiry with the channel still open), IT-011b (exclusive while heartbeating, holder named from `.meta`) |
| INV-5 | TC-019 |
| INV-6 | TC-017, TC-019 |
...
| INV-13 | `compose ps` is grep-absent from the tree (creation-only use of compose); IT-014 (the release survives an operator's `compose up -d --remove-orphans`, same id and `StartedAt`) |
| INV-14 | TC-118 (unit: label filter + config error); IT-014 (real: a worker drain runs with the release live and leaves it alone, proven by the worker being recreated) |
...
| OQ-3 | `serde_yaml` is in maintenance mode — pin it or use `serde_yml`/`saphyr` | dependency longevity | crate choice | **Resolved: `serde_norway`.** serde_yaml and the `unsafe-libyaml` it parses with were both archived in 2024; `serde_norway` is the fork that kept the whole chain alive, keeps the identical API — `Value`/`Mapping` included — so config semantics do not move, and every test passed the swap unchanged. `serde_yml` deprecated itself; `serde-saphyr` is the actively-developed option but deliberately has **no** intermediate `Value`, which is what the merge / `--set` / Lua-bridge layer is built on — that swap is a rewrite, not a dependency change |
