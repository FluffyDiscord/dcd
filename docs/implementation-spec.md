...
- **Eviction is idempotent, and it does not delete history.** An evicted release keeps its row — `dcd status` is the deploy log — and gains `reaped: true` once its teardown is complete. `evictions()` skips reaped rows. Without that flag, `evictions`/`gc_candidates` re-derive the same long-dead containers and tags from `releases[]` on *every* subsequent deploy: `docker rm -f` answers `0` for a container that is already gone and `docker image rm` answers `1 / No such image`, so the work is invisible in the exit code and grows without bound with the release history. A release is reaped only when Docker **confirmed** its container gone *and* its image is settled — removed, or never proposed because a live release still references it (a same-tag redeploy). An image that was proposed and **refused** leaves the release unreaped: the image is still on the host, and that row is the evidence dcd put it there (INV-11). `reaped` defaults to `false`, so a state written by an older dcd gets one final cleanup pass and then settles.
