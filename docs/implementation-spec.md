...
- **The stage lock does not protect cross-stage concurrency on a shared `deploy_root`.** `dcd-state.json` is explicitly cross-stage (retention subtracts other stages' tags), and v2 adds two more shared files. dcd therefore takes a short-lived `{deploy_root}/.dcd.state.lock` around every read-modify-write of the state file, and names the per-stage generated files `dcd-image-override.{stage}.yml` and `dcd-release-run.{stage}.yml`. Upload staging files are `path.tmp.{pid}.{release_id}`, removed on the error path, so two writers never collide on a fixed `.tmp` name.
...
dcd renders this into a **one-service compose document** at `{deploy_root}/dcd-release-run.yml`, written and uploaded by `sync` and appended as a `-f` **before** `dcd-image-override.yml`. The rendered service is named `{project}-release`, and that synthetic name is the key used for `Release.images`, `retention.keep_images` and `gc_logicals()` (§5.4), and the name `--image` addresses. The engine then runs the **same** `compose run` primitive against it — one creation path, one test surface.
...
- Upload every `compose.files` entry, plus `release.run.env_file` when it is a local path, to `{deploy_root}` *(Mutate)*, each via the atomic `cat > path.tmp && mv path.tmp path` of §2.3.
...
`dcd init` writes a runnable `./dcd.yaml` skeleton; `--with-plugin` also writes `plugins/app.lua`. Refuses if `dcd.yaml` exists unless `--force`. The emitted skeleton is fixed (snapshot-tested, §10).
...
| IT-010 | **no secret reaches the target's argv** (INV-12) | ssh fixture; chain carries a marker secret; a sleep is injected into `start:black` | while black is being created, `ps -eo args` **inside the fixture** contains no marker; `docker inspect` shows the value in container config | as above |
| IT-011 | **the remote lock releases itself** (INV-4) | ssh fixture; start a deploy, `SIGKILL` dcd mid-run | the lock is gone within the lease with **no** `unlock`; a second deploy acquires it. Variant: hold the channel but stop heartbeats → released within `getLockLeaseSeconds()` | fixture drop-guard |
| IT-011b | **the lock is exclusive while heartbeating** | ssh fixture; two deploys in parallel | one runs; the other exits 3 naming host/pid/start from `.meta` | as above |
| IT-012a | **transport loss pre-cutover** | ssh fixture; a `before_pull` hook kills sshd | exit **6** with `connection to <target> lost after pull`; red still serving | as above |
| IT-012b | **transport loss post-cutover** | ssh fixture; a `before_migrate_after` hook kills sshd | exit **4** (phase wins over transport); black live | as above |
...
| IT-014 | **release container survives compose reconciliation** (INV-13/14) | deploy, then run `compose up -d --remove-orphans` and a worker drain | the release container is still running and was never `docker stop`ped by worker discovery | as above |
| IT-015 | **restart policy is applied** (§7.6) | deploy, then inspect the black | `.HostConfig.RestartPolicy.Name` == the configured policy, not `no` | as above |
...
| INV-4 | TC-023, IT-005 (local flock), IT-011 (remote lease releases itself on kill AND on heartbeat loss), IT-011b (exclusive while heartbeating) |
| INV-5 | TC-019 |
| INV-6 | TC-017, TC-019 |
...
| INV-12 | TC-104 (unit: no value in argv), IT-010 (real: no marker in the target's `ps`) |
| INV-13 | IT-014 (release container survives `compose up --remove-orphans`) |
| INV-14 | TC-118 (unit: label filter + config error), IT-014 (real: worker drain does not stop the release container) |
